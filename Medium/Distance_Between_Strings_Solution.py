...
def shortestDistance(document, wordl, word2):
...
# todo: determine why tests are failing 
    ...
    word2Loc = 0 
    for word in words: 
        if word.lower() == wordl.lower():
            wordlLoc = index + (len(word)/2.0)
        elif word.lower() == word2.lower():
            word2Loc = index + (len(word)/2.0) 
        if wordlLoc > 0 and word2Loc > 0:
            current = math.fabs(wordlLoc - word2Loc)
            if current < shortest:
                shortest = current 
        index += len(word) + 1 
    if wordlLoc == 0 or word2Loc == 0: return -1 
    return shortest; 
